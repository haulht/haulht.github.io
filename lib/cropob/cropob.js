	...
	function Mask(x, y, width, height, zoom, angle, type, image_width, image_height, image_x, image_y) {
 	...
 	$.widget("ui.croima", $.ui.mouse, {
		...
		_createUI: function() {
			...
			if(this.options.enable_button_group != 2) {
				// button zoom origin
				$("<button>", {
			...
			if(this.options.enable_button_group % 2 == 0) {
				// button rotate
				$("<button>", {
				...
				// button rotate right
				$("<button>", {
					"html": this.options.icon_rotate_right,
					"class": "ob-croima-button ob-croima-rotate-right",
					'type': 'button',
				}).bind("mousedown touchstart", function () {
					me.rotateRight();
					//me._setAngle(me.image_value.angle + 1/18 * Math.PI);
				}).appendTo(div_rotate);

				// button rotate left
				$("<button>", {
					"html": this.options.icon_rotate_left,
					"class": "ob-croima-button ob-croima-rotate-left",
					'type': 'button',
				}).bind("mousedown touchstart", function () {
					//me.resetAction();
					me.rotateLeft();
				...
				$("<span>", {
					"html": "0",
					"class": "ob-croima-label ob-croima-label-angle",
				}).appendTo(div_rotate);
